...
initializeShinyApp = function(dest = './', theme = "SALURBAL"){
  ...
  dest_output = paste0(dest_search,"/")
  ...
  purrr::map(c("CSS","HTML","www","R","R/Modules"),~dir.create(paste0(dest_output,.x), recursive = TRUE,showWarnings=T))
  ...
  purrr::walk(endpoints,~{
    download.file(url = paste0(baseURL,.x),
                  destfile = paste0(dest_output,.x), mode = 'wb')
    print(paste0("Import: ",.x))
  })

  ## 5. Open new app file
  print(paste0("* ShinyUHC app successfully initialized at ",paste0(getwd(),'/',dest_output,'app.R')))
